...
public class PoolManager : MonoBehaviour 
{
	...
	public PoolingObjects[] peloadObjects; 					// List of objects that should be preloaded to the pool
	...
	void Start () 
	{
		...
		{
			if (peloadObjects[i].customName == "") 
				peloadObjects[i].customName = peloadObjects[i].objectPrefab.name;

			if (peloadObjects[i] != null)
				for (int j=0; j < peloadObjects[i].quantity; j++)
				{
					// Instantiate object and reset it transformation. Assign it as child to Pool
					newObj = Instantiate(peloadObjects[i].objectPrefab) as GameObject;
					newObj.name = peloadObjects[i].customName;

					// Add PooledObject script, if needed		 
					if (peloadObjects[i].addAutoPoolScript)
					{
	...
	public void PoolObject (GameObject _object, bool PreloadedTypeOnly)
	{
		...
		if (_object)
			if (!PreloadedTypeOnly) 
			{
				...
				for (int i=0; i<peloadObjects.Length; i++)
				{
					if(peloadObjects[i].customName == _object.name)
					{
						ResetObjectTransform(_object);
						_object.SetActive(false);
						_object.transform.parent = transform;
